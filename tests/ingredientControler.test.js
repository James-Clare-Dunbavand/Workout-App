const {
    pullIngredientInformationUsda,
    testFunc,
} = require("../controlers/ingredientControler.js");

jest.mock("../utils/ingredientUtils.js", () => ({
    ...jest.requireActual("../utils/ingredientUtils.js"),
    fetchFdcIngredients: jest.fn(),
    formatFdcIngredients: jest.fn(),
}));
const ingredientUtils = require("../utils/ingredientUtils.js");
jest.spyOn(ingredientUtils, "addManualIngredientData");
...
test("Add ingredient by calling usda if available then formating data and adding ingredient to database", async () => {
    //
    debugger;
    const requestBody = {
        body: {
    ...
    ingredientUtils.fetchFdcIngredients.mockReturnValue(
        testResponseFdcIngredients,
    );
    ingredientUtils.formatFdcIngredients.mockReturnValue(formatedFdcResponse);

    const result = await pullIngredientInformationUsda(requestBody, "");
    expect(ingredientUtils.fetchFdcIngredients).toHaveBeenCalledWith([
        170457, 790646,
    ]);

    expect(ingredientUtils.formatFdcIngredients).toHaveBeenCalledWith(
        testResponseFdcIngredients,
    );

    expect(ingredientUtils.addManualIngredientData).toHaveBeenCalledWith(
        formatedFdcResponse,
        dataMap,
    );
